...
def login_to_website(base_url, login_path, username, password):
    ...
    while 1:
        response = session.get(base_url+login_path, headers=head )
        while len(response.text) <= 200:
            # session = requests.Session()
            # Step 1: 取得登入頁面，並抓取 CSRF token（如需要）
            response = session.get(base_url + login_path ,headers=head )
        response = session.get(base_url + '/pic.aspx' , headers=head )
        soup = BeautifulSoup(response.text, 'html.parser')
        img = soup.find_all('input' , id ="__VIEWSTATE")
        value = img[0].get('value')
        if value != None:
            break
